...
def main():
    ...
    while not credit_card_is_valid:
        ...
        while True:
            product_sum_lesser_ten = ((credit_card_number_copy  % 10) * 2) < 10
            if product_sum_lesser_ten :
                individual_digit_of_product_sum = individual_digit_of_product_sum + (credit_card_number_copy % 10) * 2
            else: 
                individual_digit_of_product_sum = individual_digit_of_product_sum + (((credit_card_number_copy % 10) * 2) % 10) + ((((credit_card_number_copy % 10) * 2) / 10) % 10)
                
            credit_card_number_copy = int(credit_card_number_copy / 100)
            if not credit_card_number_copy > 0 or (int(credit_card_number_copy) / 10) > 10:
                break
