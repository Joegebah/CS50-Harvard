...
-- AND name != "Jose" AND name != "Barbara"     --Filtering only the relevant interviews
...
-- LEFT JOIN atm_transactions ON bank_accounts.account_number = atm_transactions.account_number
...
-- *!determined the id of fiftyville airport, id == 8!*
...
-- *!determined ealierst flight in the next day, id = 36, destination_id = 4
...
-- *!determined every person who had activity on exit in the bakery
...
-- SELECT name FROM people 
-- LEFT JOIN phone_calls ON people.phone_number = phone_calls.caller 
...
-- AND month == 7 AND day == 28 AND hour == 10 AND minute > 0 
-- AND minute < 25 AND activity = "exit"))) 
-- AND duration < 60 AND year == 2021 AND month == 7 AND day == 28;
...
-- *!After examining and considering both tables, there is one name which appears in both
-- tables,... Bruce.!*

-- *!Bruce must be the thief of the CS50 duck. Now the
-- accomplice...!*
...
-- *!Bruce's accomplice is Robin! After examining on whom
-- Bruce called at given time frame AND the duration
-- of the call, the data base outputs clearly that
-- Robin is Bruce's accomplice.!*

-- *!Determine the destination city or airport!*
